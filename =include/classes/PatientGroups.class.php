 ...
 *	Class PatientGroups
 *  -------------- 
 *  Description : encapsulates patient groups methods and properties
 *	Written by  : ApPHP
 *	Version     : 1.0.0
 *  Updated	    : 07.03.2011
 ...
 *	__destruct              GetAllGroupsByPatients
 *	
 *	
 **/


class PatientGroups extends MicroGrid {
	...
	{		
		...
		$this->primaryKey 	= 'id';
		$this->tableName 	= TABLE_PATIENT_GROUPS;
		$this->dataSet 		= array();
		$this->error 		= '';
		$this->formActionURL = 'index.php?admin=mod_patients_groups';
		...
		$this->arrAddModeFields = array(		    
			...
			'description' => array('title'=>_DESCRIPTION, 'type'=>'textarea', 'width'=>'310px', 'height'=>'90px', 'editor_type'=>'simple', 'readonly'=>false, 'default'=>'', 'required'=>false, 'validation_type'=>'', 'validation_maxlength'=>'255', 'unique'=>false),
	...
	/**
	 *	Get all groups by patients
	 */
	public static function GetAllGroupsByPatiens()
	{
		$sql = 'SELECT ug.id, ug.name, ug.description,
					(SELECT COUNT(*) FROM '.TABLE_PATIENTS.' u WHERE u.group_id = ug.id AND u.is_active = 1 AND u.email_notifications = 1 AND u.email != "") as patients_count
				FROM '.TABLE_PATIENT_GROUPS.' ug				
