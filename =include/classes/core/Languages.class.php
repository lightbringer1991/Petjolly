 ...
 *	Class Languages 
 *  -------------- 
 *  Description : encapsulates languages methods and properties
 *	Written by  : ApPHP
 *	Version     : 1.0.7
 *  Updated	    : 28.12.2013
 ...
 *      - added TABLE_MEMBERSHIP_PLANS_DESCRIPTION
 *      - added more tables for MedicalAppointment
 *  1.0.5
 *      - added lc_time_name
...
class Languages extends MicroGrid {
	...
	{		
		...
		$this->arrAddModeFields = array(		    
			...
			'lang_dir'     => array('title'=>_HDR_TEXT_DIRECTION, 'type'=>'enum', 'required'=>true, 'width'=>'', 'readonly'=>false, 'default'=>'ltr', 'source'=>$arr_text_directions, 'unique'=>false, 'javascript_event'=>''),
			...
			'used_on'      => array('title'=>_USED_ON, 'type'=>'enum', 'required'=>true, 'width'=>'', 'readonly'=>false, 'default'=>'global', 'source'=>$arr_used_on, 'unique'=>false, 'javascript_event'=>''),
		...
		$this->arrEditModeFields = array(
			...
			'icon_image'   => array('title'=>_ICON_IMAGE, 'type'=>'image',    'required'=>false, 'width'=>'210px', 'readonly'=>false, 'target'=>'images/flags/', 'no_image'=>'no_image.gif', 'random_name'=>false, 'overwrite_image'=>false, 'unique'=>false, 'image_width'=>'24px', 'image_height'=>'17px', 'thumbnail_create'=>false, 'thumbnail_field'=>'', 'thumbnail_width'=>'16px', 'thumbnail_height'=>'11px', 'file_maxsize'=>'100k'),
	...
	public function BeforeDeleteRecord()
	{
		...
		$this->d_language_abbrev = $result_current['abbreviation'];
		$this->d_priority_order = $result_current['priority_order'];

		if($this->d_languages_count > 1){
			if($result_current['is_default'] == '1'){
				$this->error = _DEFAULT_LANG_DELETE_ALERT;
				return false;
			}		
		}else{
	...
	private function CopyDataToNewLang($from_abbrev, $to_abbrev)
	{
		...
		if(self::$PROJECT == 'BusinessDirectory'){
			...
			// clone data for Membership Plans table
			$sql = 'INSERT INTO '.TABLE_MEMBERSHIP_PLANS_DESCRIPTION.' (membership_plan_id, language_id, name, description)
							                         (SELECT membership_plan_id, \''.$to_abbrev.'\', name, description FROM '.TABLE_MEMBERSHIP_PLANS_DESCRIPTION.' WHERE language_id = \''.$from_abbrev.'\')';
            database_void_query($sql);			                                                     

			// clone data for Clinic table
			$sql = 'INSERT INTO '.TABLE_CLINIC_DESCRIPTION.' (clinic_id, language_id, name, address, description)
							                         (SELECT clinic_id, \''.$to_abbrev.'\', name, address, description FROM '.TABLE_CLINIC_DESCRIPTION.' WHERE language_id = \''.$from_abbrev.'\')';
            database_void_query($sql);			                                                     

			// clone data for Insurances table
			$sql = 'INSERT INTO '.TABLE_INSURANCES_DESCRIPTION.' (insurance_id, language_id, name, description)
							                         (SELECT insurance_id, \''.$to_abbrev.'\', name, description FROM '.TABLE_INSURANCES_DESCRIPTION.' WHERE language_id = \''.$from_abbrev.'\')';
            database_void_query($sql);			                                                     

			// clone data for Specialities table
			$sql = 'INSERT INTO '.TABLE_SPECIALITIES_DESCRIPTION.' (speciality_id, language_id, name, description)
							                         (SELECT speciality_id, \''.$to_abbrev.'\', name, description FROM '.TABLE_SPECIALITIES_DESCRIPTION.' WHERE language_id = \''.$from_abbrev.'\')';
            database_void_query($sql);			                                                     
		}
	...
	{	
		...
		if(self::$PROJECT == 'BusinessDirectory'){
			...
			$sql = 'DELETE FROM '.TABLE_MEMBERSHIP_PLANS_DESCRIPTION.' WHERE language_id = \''.$lang_abbrev.'\'';
			database_void_query($sql);
			$sql = 'DELETE FROM '.TABLE_CLINIC_DESCRIPTION.' WHERE language_id = \''.$lang_abbrev.'\'';
			database_void_query($sql);            
			$sql = 'DELETE FROM '.TABLE_INSURANCES_DESCRIPTION.' WHERE language_id = \''.$lang_abbrev.'\'';
			database_void_query($sql);
			$sql = 'DELETE FROM '.TABLE_SPECIALITIES_DESCRIPTION.' WHERE language_id = \''.$lang_abbrev.'\'';
			database_void_query($sql);            
		}
