...
class Settings {
	...
	function __construct()
	{
		$this->error = '';
		$this->template = '';
		$this->provider_template = '';
		$this->member_template = '';
	...
	public function LoadData()
	{
		...
		if($this->res){
			...
			$this->provider_template = $this->res['provider_template'];
			$this->member_template = $this->res['member_template'];
		}
	}
	...
	/**
	 *	Returns provider template name
	 */
	public function GetProviderTemplate()
	{
		return decode_text($this->provider_template);
	}/**
	 *	Returns template name
	 */
	public function GetMemberTemplate()
	{
		return decode_text($this->member_template);
	}
	...
	/**
	 *	Sets site member template
	 *		@param $template - template name
	 */
	public function SetMemberTemplate($template = '')
	{
		// check if this is a DEMO
		if(strtolower(SITE_MODE) == 'demo'){ $this->error = defined('_OPERATION_BLOCKED') ? _OPERATION_BLOCKED : 'This operation is blocked in Demo Version!'; return false; }
		
		$sql = 'UPDATE '.TABLE_SETTINGS.' SET member_template = \''.encode_text($template).'\'';
		if (database_void_query($sql)){
			$this->member_template = $template;
			return true;
		} else {
			$this->error = _TRY_LATER;
			return false;
	...
	/**
	 *	Sets site provider template
	 *		@param $template - template name
	 */
	public function SetProviderTemplate($template = '')
	{
		// check if this is a DEMO
		if(strtolower(SITE_MODE) == 'demo'){ $this->error = defined('_OPERATION_BLOCKED') ? _OPERATION_BLOCKED : 'This operation is blocked in Demo Version!'; return false; }
		
		$sql = 'UPDATE '.TABLE_SETTINGS.' SET provider_template = \''.encode_text($template).'\'';
		if (database_void_query($sql)){
			$this->provider_template = $template;
			return true;
		} else {
			$this->error = _TRY_LATER;
			return false;
