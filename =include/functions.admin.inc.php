 ...
 *  	@param $page
 **/
function get_chart_changer($tabid, $chart_type, $year, $doctor_id = '-1', $page = 'statistics')
{
	$output = '<form action="'.APPHP_BASE.'index.php?admin='.$page.'" name="frmStatistics" method="post">
		...
		<tr>
			...
			if($doctor_id != '-1'){
				$output .= _DOCTOR.':  <select name="doctor_id">';
				$output .= '<option value="">'._ALL.'</option>';
				$total_doctors = Doctors::GetAllActive();
				foreach($total_doctors[0] as $key => $val){
					$output .= '<option value="'.$val['id'].'" '.(($doctor_id == $val['id']) ? ' selected="selected"' : '').'>'.$val['first_name'].' '.$val['middle_name'].' '.$val['last_name'].'</option>';
				}
				$output .= '</select>&nbsp;&nbsp;';
			}
			$output .= '</td>
