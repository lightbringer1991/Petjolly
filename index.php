...
$login = new Login();
if(isset($_GET["submit_logout"]) && ($_GET["submit_logout"]=="logout"))
{
...
// -----------------------------------------------------------------------------
if((Application::Get('page') != '') && file_exists('page/handlers/ '.Application::Get('page').'.php')){
...
}else if((Application::Get('provider') != '') && file_exists('provider/handlers/handler_'.Application::Get('provider').'.php')){
	include_once('provider/handlers/handler_'.Application::Get('provider').'.php');	
...
}else if((Application::Get('member') != '') && file_exists('patient/handlers/handler_'.Application::Get('member').'.php')){
	if(Modules::IsModuleInstalled('patients')){	
		include_once('patient/handlers/handler_'.Application::Get('member').'.php');
	}
}
...
if($objSettings->GetParameter('caching_allowed') && !$objLogin->IsLoggedIn()){
	...
	if($objLogin->IsLoggedInAsDoctor()) {
		require_once('templates/'.Application::Get('provider_template').'/default.php');
	} else if($objLogin->IsLoggedInAsPatient()) {
		require_once('templates/'.Application::Get('member_template').'/default.php');
	} else {
...
//echo "\n".'<!-- This page was generated by ApPHP Medical Appointment Pro v'.CURRENT_VERSION.' (http://www.apphp.com/php-medical-appointment/) -->';
