...
//				__mgDoPostBack('mdo_appointments1', 'edit',eventid[1]);
	...
	?>
	  ...
	  function doSubmit(){
		...
		$.ajax({
		type:"post",
		url:"index.php?doctor=calendar&action=addapp&ajax=1",
		dataType:"html",
	   ...
	   <?PHP if(strtolower($_GET['doctorid'])!='all'){?>
		...
		$(".fc-button-appointmnts").click(function() {
			window.location='index.php?doctor=calendar&pageview=appointments';
		});

		$(".fc-button-mysechdules").click(function() {
			window.location='index.php?doctor=calendar&pageview=schedule';

		});
		$(".fc-button-timeoff").click(function() {
			window.location='index.php?doctor=calendar&pageview=timeoff';

		});
		$(".fc-button-reports").click(function() {
			window.location='index.php?doctor=calendar&pageview=reports';
		});
	...
	//	$( "#aptabs" ).tabs();
		...
		<?php if($action=='edit' && $pageview!='timeoff' && $pageview!='schedule' && $pageview!='reports'){
			...
			$('#frmMicroGrid_mdo_appointments_update').attr('action','index.php?doctor=calendar&pageview=appointments');
			$("input[name=btnCancel]")[0].onclick = null;
			$("input[name=btnCancel]").click(function() { window.location='index.php?doctor=calendar&pageview=appointments';});
...
//						frmObj.action="index.php?doctor=calendar&pageview=<?php echo $pageview;?>";
