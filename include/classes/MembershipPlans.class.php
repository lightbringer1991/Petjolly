...
class MembershipPlans extends MicroGrid {
	...
	public static function DrawPlans($draw = true)
	{
		...
		$result = database_query($sql, DATA_AND_ROWS, ALL_ROWS);
		if($result[1] > 0){
			$output .= '<form name="frmMembershipForm" id="frmMembershipForm" action="index.php?provider=membership_prepayment" method="post">';
	...
	{
        $where_clause = "(membership_plan_id = 4 AND membership_expires != '0000-00-00' OR membership_plan_id != 4 AND '".date('Y-m-d H:i:s')."' > membership_expires)";
        //$sql = "SELECT *
        //        FROM ".TABLE_DOCTORS."
        //        WHERE
        //            (".$where_clause.") AND 
