...
class Timeoffs extends MicroGrid {
	...
	{		
		...
		$this->error 		= '';
		if($account_type == 'me'){
			$this->formActionURL = 'index.php?provider=timeoff_management';
		}else{
    ...
    /*
     * Get all time off of a provider between start and end date period
     */
    public static function getTimeOff($provider_id, $startDate, $endDate) {
        $sql = "SELECT `description`, `date_from`, `time_from`, `date_to`, `time_to` FROM `meda_timeoffs`
                WHERE `doctor_id`=$provider_id AND `date_from`>='$startDate' AND `date_to`<='$endDate'";
        $result = database_query($sql, DATA_AND_ROWS, ALL_ROWS);
        $output = array();
        foreach ($result[0] as $r) {
            $aRow = array(
                'description' => $r['description'],
                'start' => $r['date_from'] . " " . $r['time_from'],
                'end' => $r['date_to'] . " " . $r['time_to']
            );
            array_push($output, $aRow);
        }
        return $output;
    }
