...
class DoctorSpecialities extends MicroGrid {
	...
	{		
		...
		$this->error 		= '';
		if($account_type == 'me'){
			$this->formActionURL = 'index.php?provider=my_specialities';	
		}else{
		...
		$this->EDIT_MODE_SQL = 'SELECT
							...
							FROM '.$this->tableName.' ds
								INNER JOIN '.TABLE_SPECIALITIES_DESCRIPTION.' sd ON ds.speciality_id = sd.id
    ...
    public static function GetSpecialitiesByProviderId($id) {
        $sql = "SELECT `" . TABLE_DOCTOR_SPECIALITIES . "`.`speciality_id`, `name` FROM `" . TABLE_DOCTOR_SPECIALITIES . "`, `" . TABLE_SPECIALITIES_DESCRIPTION . "` WHERE `doctor_id`=$id AND `" . TABLE_DOCTOR_SPECIALITIES . "`.`speciality_id`=`" . TABLE_SPECIALITIES_DESCRIPTION . "`.`speciality_id`";
