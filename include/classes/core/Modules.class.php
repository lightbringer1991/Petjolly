				...
				if($this->modules[0][$i]['is_system'] == '1'){					
					if($objLogin->IsLoggedInAs($this->modules[0][$i]['settings_access_by'])){
						$modules_output .= '<div style="width:120px;float:'.Application::Get('defined_left').';text-align:center;margin:5px;">
							...
							<div><img src="images/modules_icons/'.$this->modules[0][$i]['icon_file'].'" title="'.@decode_text(constant($this->modules[0][$i]['description_const'])).'" alt="icon" style="cursor:help;margin:2px;border:1px solid #dedede"></div>
							<div>'.(($this->modules[0][$i]['is_installed'] == 1) ? '<img src="images/success_sign.gif" style="position:absolute;'.$margin.'" alt="installed">' : '<img src="images/error_sign.gif" style="position:absolute;'.$margin.'" alt="removed">').'</div>
			...
			if($this->modules[0][$i]['show_on_dashboard'] == '1' && ($this->modules[0][$i]['is_installed'] == 1)){
				if($objLogin->IsLoggedInAs($this->modules[0][$i]['settings_access_by'])){
					...
					$output .= '<div style="width:120px;float:'.Application::Get('defined_left').';text-align:center;margin:5px;">
						<div><a href="'.$href.'" title="'._CLICK_TO_VIEW.'"><b>'.decode_text(constant($this->modules[0][$i]['name_const'])).'</b></a></div>
						<div><a href="'.$href.'" title="'._CLICK_TO_VIEW.'"><img src="images/modules_icons/'.$this->modules[0][$i]['icon_file'].'" alt="icon" style="margin:2px;border:1px solid #dedede"></a></div>						
					</div>';
				}
			}
		}
