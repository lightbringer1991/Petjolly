...
class Login {
	 ...
	 */
	public function DoLogin($user_name, $password, $remember_me = '', $do_redirect = true)
	{

		global $objSession;
		
		$ip_address = get_current_ip();
		$redirect_url   = isset($_REQUEST['task']) ? prepare_input($_REQUEST['task']) : '';
	...
	public function IsLoggedInAsDoctor($check = true)
	{
		if(!$this->IsLoggedIn() && $check) return false;
		$account_type = Session::Get('session_account_type');
		if($account_type == 'doctor' or $account_type == 'provider') return true;
		return false;
	}
	...
	{
		if($this->GetLoggedType() == 'doctor' or $this->GetLoggedType() == 'provider'){
			$table = TABLE_DOCTORS;
		}else{
	...
	public function DrawLoginLinks()
	{
		...
		if($this->IsLoggedInAsAdmin()){
			...
			echo   "<div id='categories' class='left-box-container'>
						<h3 id='categoriesHeading' class='side_box_heading'><a href='index.php?admin=setup'>Business Setup</a></h3>
					</div>";
		}
	...
	private function PrepareLink($href, $link, $params='', $class='', $href_array=array())
	{
		...
		if($this->GetLoggedType() == 'patient'){
			$logged_as = 'patient';	
		}else if($this->GetLoggedType() == 'doctor' or $this->GetLoggedType() == 'provider'){
			$logged_as = 'provider';	
		}else{
			$logged_as = 'admin';
		}		
