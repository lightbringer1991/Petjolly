...
class PaymentIPN {
	...
	{
		$nl = "\n";		
		$output  = $nl.'<form action="index.php?provider=order_proccess" method="post">';
		...
		$output .= $nl.'&nbsp; - '._OR.' - &nbsp;';
		$output .= $nl.'<a href="javascript:void(0);" onclick="javascript:appGoTo(\'doctor=membership_plans\')">'._BUTTON_CANCEL.'</a>';
		$output .= $nl.'</td>';
		$output .= $nl.'</tr>';
		$output .= $nl.'</table>';							
		$output .= $nl.'</form>';
