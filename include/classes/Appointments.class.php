...
class Appointments extends MicroGrid {
	...
	function __construct() {
		...
		if($objLogin->IsLoggedInAsPatient()) {
			$this->formActionURL = 'index.php?member=my_appointments';		
		} else if($objLogin->IsLoggedInAsDoctor()) {
			$this->formActionURL = 'index.php?provider=appointments';		
		} else {
		...
		if($result[1] > 0) {
			if($result[0]['status'] == '1') {
				$arr_statuses_edit = array('1'=>'<style="color:#00a300">'._VERIFIED.'</span>',
		...
		$this->VIEW_MODE_SQL = 'SELECT '.$this->tableName.'.'.$this->primaryKey.',
                                    ...
                                    (   CASE ' . $this->tableName . '.created_by
                                                    WHEN "admin" THEN (SELECT CONCAT(first_name, " ", last_name, " (admin)")  from ' . TABLE_ACCOUNTS . ' where ' . TABLE_ACCOUNTS . '.id = ' . $this->tableName . '.created_by_id)
                                                    WHEN "customer" THEN (SELECT CONCAT(first_name, " ", last_name, " (customer)")  from ' . TABLE_PATIENTS . ' where ' . TABLE_PATIENTS . '.id = ' . $this->tableName . '.created_by_id)
                                                    WHEN "provider" THEN (SELECT CONCAT(first_name, " ", last_name, " (provider)")  from ' . TABLE_DOCTORS .' where ' . TABLE_DOCTORS . '.id = ' . $this->tableName . '.created_by_id)
                                        END
                                    ) AS created_by_name,
									(SELECT
											a.appointment_date
									...
									CONCAT("<a href=\"javascript:void(\'doctor|view\')\" onclick=\"appAjaxPopupWindow(\'popup.ajax.php\',\'doctor\',\'", '.TABLE_DOCTORS.'.id, "\',\''.Application::Get('token').'\',\''.Application::Get('lang_dir').'\')\">", '.TABLE_DOCTORS.'.first_name, " ", '.TABLE_DOCTORS.'.middle_name, " ", '.TABLE_DOCTORS.'.last_name, "</a><br><span class=gray>", sd.name, "</span>") as doctor_name,
                                    ...
                                    CONCAT("<img src=\"images/appointment.png\" alt=\"icon\" />") new_appointment_link,
...
//			'b_country'          => array('title'=>_COUNTRY, 'type'=>'label', 'align'=>'center', 'width'=>'', 'height'=>'', 'maxlength'=>'', 'visible'=>$show_country_column),
			'appointment_number' => array('title'=>_APPOINTMENT_NUMBER, 'type'=>'label', 'align'=>'left', 'width'=>'100px', 'sortable'=>true, 'nowrap'=>'', 'visible'=>true, 'tooltip'=>'', 'maxlength'=>'', 'format'=>'', 'format_parameter'=>''),
...
//			'd_confirm_reminder_sent' => array('title'=>'DC', 'header_tooltip'=>_DOCTOR_ARRIVAL_REMINDER_EMAIL, 'type'=>'enum',  'align'=>'center', 'width'=>'35px', 'sortable'=>true, 'nowrap'=>'', 'visible'=>$reminders_visible, 'source'=>$arr_is_sent_vm),
            'created_by_name'         => array('title'=>_CREATED_BY, 'type'=>'label',  'align'=>'left', 'width'=>'90px', 'sortable'=>true, 'nowrap'=>'', 'visible'=>true),
			'status'             => array('title'=>_STATUS, 'type'=>'enum',  'align'=>'left', 'width'=>'90px', 'sortable'=>true, 'nowrap'=>'', 'visible'=>true, 'source'=>$arr_statuses_view),
			'new_appointment_link' => array('title'=>'', 'type'=>'label', 'visible'=>false),
		...
		if($objLogin->GetLoggedType() == 'patient'){
			...
			$this->arrViewModeFields['new_appointment_link'] = array('title'=>'', 'type'=>'link',  'align'=>'left', 'width'=>'', 'sortable'=>true, 'nowrap'=>'', 'visible'=>true, 'tooltip'=>_NEW_APPOINTMENT, 'maxlength'=>'', 'href'=>'index.php?page=find_doctors&docid={doctor_id}', 'target'=>'');
		}
...
//		$output .= draw_hidden_field('docid', $docid, false);
       ...
       if($doctor_info[1]){
            ...
            $output .= '<div class="appointment_row" style="display:none">'.$nl;        
  ...
  /* if($objLogin->IsLoggedInAsPatient()) */ $output .= '<input class="button" type="button" onclick="redirect()" value="'._BUTTON_BACK.'"> ';
		...
		$output .= '</form><script>function redirect(){
			window.location = "index.php?page=find_doctors";
		}</script>';
...
//			$output .= '<input type="radio" name="user_type" value="new" Checked>New User</br>'.$n1;
			
//			$output .= '<input type="radio" name="user_type" value="existing">Existing User'.$n1;
		...
		//	$output .= '<tr><td><input type="radio" name="user_type" value="new" Checked>New User</td></tr>'.$n1;
			
	//		$output .= '<tr><td><input type="radio" name="user_type" value="existing">Existing User</td></tr>'.$n1;
	...
	public static function DrawAppointmentDetailsForGuest($params = array(), $draw = true)
	{
		global $objLogin;
...
//		$output .= '<form action="index.php?'.$action.'" method="post">';
	...
	//	$output .= draw_hidden_field('schid', $schid, false);
		...
		$output .= '<tr><td><input type="radio" name="user_type" value="new" onClick="display_GuestAction(\'0\')">New User</td></tr>'.$n1;
		$output .= '<tr><td colspan="3" id="guest_register" style="display:none;">';
		$output .= '<form class="guest_register" action="index.php?member=create_guest" method="post">
			...
			<tr><td width="38%" align="">'._FIRST_NAME.'</td><td><span class="mandatory_star">*</span></td>
				<td nowrap="nowrap"><input type="text" id="first_name" name="first_name" size="32" maxlength="32" value="" autocomplete="off" /></td></tr>
			<tr><td align="">'._LAST_NAME.'</td><td><span class="mandatory_star">*</span></td>
				<td nowrap="nowrap"><input type="text" id="last_name" name="last_name" size="32" maxlength="32" value="" autocomplete="off" /></td></tr>
			<tr><td align="">'._PHONE.'</td><td></td>
				<td nowrap="nowrap"><input type="text" id="phone" name="phone" size="32" maxlength="32" value="" autocomplete="off" /></td></tr>
			<tr><td align="">'._EMAIL_ADDRESS.'</td><td><span class="mandatory_star">*</span></td>
				<td nowrap="nowrap"><input type="text" id="email" name="email" size="32" maxlength="70" value="" autocomplete="off" />'._ENTER_EMAIL_ADDRESS.'</td></tr>
			<tr><td align="">'._USERNAME.'</td><td class="mandatory_star">*</td>
				<td nowrap="nowrap"><input type="text" id="frmReg_user_name" name="user_name" size="32" maxlength="32" value="" autocomplete="off" /></td></tr>
			<tr><td>'._PASSWORD.':</td><td class="mandatory_star">*</td>
				<td><input type="password" name="password" id="password" maxlength="20" autocomplete="off" value="" /></td></tr>
				
				<tr><td>'._REPASSWORD.':</td><td class="mandatory_star">*</td>
				<td><input type="password" name="rpassword" id="repassword" maxlength="20" autocomplete="off" value="" /></td></tr>
				
			<tr><td nowrap height="1px"></td></tr>
			<tr><td></td></tr>
			<tr><td>
			<input class="form_button" type="submit" name="submit" value="'._SUBMIT.'" /></td></tr>
			<input type="hidden" name="act" value="create" />
			'.draw_hidden_field('task', 'appointment_verify', false).'
			</table></form></td></tr>';
		$output .= '<tr><td><input type="radio" name="user_type" value="existing" onClick="display_GuestAction(\'1\')">Existing User</td></tr>'.$n1;
		$output .= '<tr><td style="display:none;" id="guest_login"><form class="patient_login" action="index.php?member=login" method="post">
			'.draw_hidden_field('submit_login', 'login', false).'
			'.draw_hidden_field('type', 'patient', false).'
			'.draw_token_field(false).'
			<table class="tblSideBlock" border="0" cellspacing="1" cellpadding="1">
			...
			<tr><td nowrap height="1px"></td></tr>
			<tr><td>';
		$output .= draw_hidden_field('task', 'appointment_verify', false);
		$output .= draw_hidden_field('docid', $docid, false);
		$output .= '<input class="form_button" type="submit" name="submit" value="'._BUTTON_LOGIN.'" />';
		$output .= '</td></tr><tr><td></td></tr>';
	    $output .= '</table></form>';
		$output .= '</td></tr>';
		$output .= '<tr><td nowrap="nowrap" height="10px"></td></tr>'.$nl;
	//	$output .= '<tr><td align="'.Application::Get('defined_left').'"><input class="button" type="submit" value="'._BOOK_NOW.'"></td></tr>'.$nl;
...
//		$output .= '</form>';
	...
	public static function DrawAppointmentGuest($draw = true){
		...
		$appointment_details['start_time'] = isset($_POST['start_time']) ? prepare_input($_POST['start_time']) : '';
		$appointment_details['duration']   = isset($_POST['duration']) ? prepare_input($_POST['duration']) : '';	
		$output = Patients::DrawGuestFormBlock($draw);
	...
	public static function DoAppointment($params = array()) {
		...
		if($result[1] > 0) {
			...
			if(database_void_query($sql)) {
				// update pet notes
				$petNotes = isset($params['pet_notes']) ? $params['pet_notes'] : '';
				$sql = "UPDATE `meda_pets` SET `notes`='$petNotes' WHERE `id`={$params['patient_id']}";
...
*/
	...
	public static function SendAppointmentEmail($type = '', $appt_number = '') {
        ...
        $sql = 'SELECT
					...
					d.business_name as doc_business_name,
					d.medical_degree as doc_medical_degree,
...
*/
				...
				if($result[0][$i]['pat_email'] != ''){
					...
					send_email(
						...
						array(
							...
							'{BASE URL}'   => APPHP_BASE.'index.php?member=confirm_appointment&n='.$result[0][$i]['appointment_number'],
				...
				if($result[0][$i]['doc_email'] != ''){
					...
					send_email(
						...
						array(
							...
							'{BASE URL}'   => APPHP_BASE.'index.php?provider=login',
	...
	private static function DrawAppointmentTable($result, $date_format, $time_format, $draw = true) {
        ...
        // get services
        $servicesID = explode(",", $result['service_list']);
        $services = array();
        foreach ($servicesID as $s) {
            $aService = Services::getServiceById($s);
            if ($aService != null) { array_push($services, $aService); }
        }

        // get packages
        $packagesID = explode(",", $result['package_list']);
        $packages = array();
        foreach ($packagesID as $p) {
            $aPackage = Packages::getPackageById($p);
            if ($aPackage != null) { array_push($packages, $aPackage); }
        }
        ...
        $output .= "</td></tr>";
        $output .= '<tr><td>' . _PROVIDER_PACKAGES . ': </td><td>';
        foreach ($packages as $p) {
            $output .= $p -> getName() . "<br />";
        }
        $output .= "</td></tr>";
//		$output .= '<tr><td>'._WHERE.' ('._ADDRESS.'): </td><td>'.$result['doc_address'].'</td></tr>';
        $output .= '<tr><td>'._WHERE.' ('._ADDRESS.'): </td><td>' . $result['b_address'] . ", " . $result['b_city'] . ", " . $result['b_state'] . ", " . $result['b_country'] . ", " . $result['b_zipcode'] . '</td></tr>';
...
*/
    ...
    public static function getAllTodaysAppointmentByProvider($id) {
        $sql = "SELECT `meda_appointments`.`id`, `appointment_date`, `appointment_time`, `first_name`, `last_name`, `service_list`, `package_list`
                    FROM `meda_appointments`, `meda_patients`
                    WHERE `doctor_id`=$id AND `patient_id`=`meda_patients`.`id` AND DATE(`appointment_date`)=DATE(NOW()) AND `status`!=2";
        return database_query($sql, DATA_AND_ROWS, ALL_ROWS);
    }
    public static function getAllUpcomingAppointmentsInMonthByProvider($id) {
        $sql = "SELECT `meda_appointments`.`id`, `appointment_date`, `appointment_time`, `first_name`, `last_name`, `service_list`, `package_list`
                    FROM `meda_appointments`, `meda_patients`
