 ...
 **/
 ...
 /*
	...
	Date modified: 17/03/2015
	Description: added more functions to be used in Petdaddy website
	Added functions:
		GetAllCustomersByProvider
 */
...
class Patients extends MicroGrid {
...

	{
		...
		$this->EDIT_MODE_SQL = 'SELECT
									...
									IF('.$this->tableName.'.notification_status_changed = "0000-00-00 00:00:00", "<span class=gray>- never -</span>", DATE_FORMAT('.$this->tableName.'.notification_status_changed, \''.$this->sqlFieldDatetimeFormat.'\')) as notification_status_changed
	...
	// added functions to be used in petdaddy site
	public static function GetAllCustomersByProvider($providerID) {
		$query = "SELECT * FROM `meda_patients`, `providers_customers` WHERE `provider_id`=$providerID AND `id`=`customer_id` AND `is_removed`=0";
		return database_query($query, DATA_AND_ROWS, ALL_ROWS);
	}
	
	public static function removeCustomer($id) {
		$query = "UPDATE `meda_patients` SET `is_removed`=1 WHERE `id`=$id";
		database_void_query($query);
	}
...

	{
		...
		$output .= '<form class="patient_login" action="index.php?member=login" method="post">
		...
		$output .= '</td></tr>
			...
			if(ModulesSettings::Get('patients', 'allow_registration') == 'yes') $output .= '<tr><td>'.prepare_permanent_link('index.php?member=create_account', _CREATE_ACCOUNT, '', 'form_link').'</td></tr>';

			if(ModulesSettings::Get('patients', 'allow_reset_passwords') == 'yes') $output .= '<tr><td>'.prepare_permanent_link('index.php?member=password_forgotten', _FORGOT_PASSWORD, '', 'form_link').'</td></tr>';
	...
	public static function DrawGuestFormBlock($draw = true){
		global $objLogin;		
		$username = '';
		$password = '';
		$output = //draw_block_top(_AUTHENTICATION, '', 'maximized', false);
		$output .= '<form class="patient_login" action="index.php?member=create_guest" method="post">
			'.draw_hidden_field('submit_login', 'login', false).'
			'.draw_hidden_field('type', 'patient', false).'
			'.draw_token_field(false).'
			<table class="tblSideBlock" border="0" cellspacing="1" cellpadding="1">
			<tr>
				<td width="38%" align="">'._FIRST_NAME.'</td><td><span class="mandatory_star">*</span></td>
				<td nowrap="nowrap"><input type="text" id="first_name" name="first_name" size="32" maxlength="32" value="" autocomplete="off" /></td>
			</tr>
			<tr>
				<td align="">'._LAST_NAME.'</td><td><span class="mandatory_star">*</span></td>
				<td nowrap="nowrap"><input type="text" id="last_name" name="last_name" size="32" maxlength="32" value="" autocomplete="off" /></td>
			</tr>
			<tr>
				<td align="">'._PHONE.'</td><td></td>
				<td nowrap="nowrap"><input type="text" id="phone" name="phone" size="32" maxlength="32" value="" autocomplete="off" /></td>
			</tr>
			<tr>
				<td align="">'._EMAIL_ADDRESS.'</td><td><span class="mandatory_star">*</span></td>
				<td nowrap="nowrap"><input type="text" id="email" name="email" size="32" maxlength="70" value="" autocomplete="off" />'._ENTER_EMAIL_ADDRESS.'</td>
			</tr>
			<tr>
				<td align="">'._USERNAME.'</td><td class="mandatory_star">*</td>
				<td nowrap="nowrap"><input type="text" id="frmReg_user_name" name="user_name" size="32" maxlength="32" value="" autocomplete="off" /></td>
			</tr>
			<tr>
				<td>'._PASSWORD.':</td><td class="mandatory_star">*</td>
				<td><input type="password" name="password" id="password" maxlength="20" autocomplete="off" value="" /></td>
			</tr>
			<tr>
				<td nowrap height="1px"></td>
			</tr>
			<!--tr>
				<td align="">'._CONFIRM_PASSWORD.'</td>
				<td class="mandatory_star">*</td>
				<td nowrap="nowrap"><input type="password" id="frmReg_user_password2" name="user_password2" size="32" maxlength="20" value="" autocomplete="off" /></td>
			</tr-->
			<tr><td>';
			$output .= '<input class="form_button" type="submit" name="submit" value="'._SUBMIT.'" />';	
			$output .= '<input type="hidden" name="act" value="create" />';
		/*if(ModulesSettings::Get('patients', 'remember_me_allow') == 'yes'){
			$output .= '<input type="checkbox" class="form_checkbox" name="remember_me" id="chk_remember_me" value="1" /> <label for="chk_remember_me">'._REMEMBER_ME.'</label><br>';
		} */				
			$output .= '</td></tr>
