...
if(!$objLogin->IsLoggedIn() && (ModulesSettings::Get('patients', 'allow_registration') == 'yes')){
    ...
    draw_title_bar(prepare_breadcrumbs(array(_PATIENTS=>'',_REGISTRATION_CONFIRMATION=>'')));
	...
	if(!$confirmed){
		echo '<br />
		<form action="index.php?member=confirm_registration" method="post" name="frmConfirmCode" id="frmConfirmCode">
