...
?>
	...
	<form action="index.php?member=resend_activation" method="post">
			...
			<td colspan='2'>
				<?php
					if(ModulesSettings::Get('patients', 'allow_login') == 'yes'){
						echo prepare_permanent_link('index.php?member=login', _PATIENT_LOGIN).'<br />';
					}				
					if(ModulesSettings::Get('patients', 'allow_registration') == 'yes'){
						echo prepare_permanent_link('index.php?member=create_account', _CREATE_ACCOUNT);
					}
				?>
			</td>
		</tr>
