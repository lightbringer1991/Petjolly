...
?>
	...
	<form action="index.php?provider=resend_activation" method="post">
			...
			<td colspan="2">
				<?php
					if(ModulesSettings::Get('doctors', 'allow_login') == 'yes'){
						echo prepare_permanent_link('index.php?provider=login', _DOCTOR_LOGIN).'<br />';
					}				
					if(ModulesSettings::Get('doctors', 'allow_registration') == 'yes'){
						echo prepare_permanent_link('index.php?provider=create_account', _CREATE_ACCOUNT);
					}
				?>
			</td>
		</tr>
