...
if($objLogin->IsLoggedInAs('owner','mainadmin')){
	...
	if($tabid == '1_1') {
		...
		$tab_content_1 = '<form action="index.php?admin=settings" method="post">
			...
			<tr valign="top">
				<td width="150px">'._SITE_OFFLINE.': <img class="help" src="images/question_mark.png" title="'._SITE_OFFLINE_ALERT.'" alt="help" /></td>
				<td colspan="2">
			...
			<tr valign="top">
				<td width="150px">'._OFFLINE_MESSAGE.': <img class="help" src="images/question_mark.png" title="'._SITE_OFFLINE_MESSAGE_ALERT.'" alt="help" /></td>
				<td colspan="2">
		    ...
		    <tr valign="top">
			    ...
			    <td colspan="2">
				    <select name="ssl_mode">
						...
						<option '.(($params["ssl_mode"] == "3") ? "selected" : "").' value="3">'._PD_PAYMENT_MODULES.'</option>
				    </select>
			    </td>
		    </tr>
			<tr valign="top">
				<td width="150px">'._SEO_URLS.': <img class="help" src="images/question_mark.png" title="'._SEO_LINKS_ALERT.'" alt="help" /></td>
				<td colspan="2">
		...
		<legend>'._HEADERS_AND_FOOTERS.'</legend>
			...
			<table width="99%" border="0" cellspacing="5" cellpadding="5" class="tabs_table main_text">
			<tr valign="top">
				<td width="150px">'._HDR_HEADER_TEXT.' <span class="required">*</span>:</td>
				<td><textarea class="form_text" name="header_text" id="header_text" cols="54" rows="3">'.decode_text($params_visual['header_text']).'</textarea></td>
			</tr>
			<tr valign="top">
				<td width="150px">'._HDR_SLOGAN_TEXT.':</td>
				<td><textarea class="form_text" name="slogan_text" id="slogan_text" cols="54" rows="3">'.decode_text($params_visual['slogan_text']).'</textarea></td>
			</tr>
			<tr valign="top">
				<td width="150px">'._HDR_FOOTER_TEXT.':</td>
				<td><textarea class="form_text" name="footer_text" id="footer_text" cols="54" rows="3">'.decode_text($params_visual['footer_text']).'</textarea></td>
			</tr>
		...
		<legend>'._META_TAGS.'</legend>
			...
			<tr valign="top">
				...
				<td><textarea class="form_text" name="tag_title" id="tag_title" cols="54" rows="2">'.$params_visual_meta['tag_title'].'</textarea></td>
			</tr>
			<tr valign="top">
				<td width="150px">'._META_TAG.' &lt;KEYWORDS&gt;:</td>
				<td><textarea class="form_text" name="tag_keywords" id="tag_keywords" cols="54" rows="3">'.$params_visual_meta['tag_keywords'].'</textarea></td>
			</tr>
			<tr valign="top">
				<td width="150px">'._META_TAG.' &lt;DESCRIPTION&gt;:</td>
				<td><textarea class="form_text" name="tag_description" id="tag_description" cols="54" rows="3">'.$params_visual_meta['tag_description'].'</textarea></td>
			</tr>
			<tr>
		...
		'.draw_token_field(false).'
			...
			<tr valign="top">
				...
				<td colspan="2">
					<select name="date_format">
						<option'.(($params_datetime['date_format'] == 'dd/mm/yyyy') ? ' selected="selected"' : '').' value="dd/mm/yyyy">dd/mm/yyyy</option>
						<option'.(($params_datetime['date_format'] == 'mm/dd/yyyy') ? ' selected="selected"' : '').' value="mm/dd/yyyy">mm/dd/yyyy (american)</option>
					</select>
				</td>
			</tr>
			<tr valign="top">
				<td width="150px">'._TIME_FORMAT.': </td>
				<td colspan="2">
					<select name="time_format">
						<option'.(($params_datetime['time_format'] == '24') ? ' selected="selected"' : '').' value="24">24h</option>
						<option'.(($params_datetime['time_format'] == 'am/pm') ? ' selected="selected"' : '').' value="am/pm">AM/PM</option>
					</select>
				</td>
			</tr>
			<tr valign="top">
				...
				<td colspan="2">
					...
					foreach($arr_time_zones as $key => $val){
						$tab_content_3 .= '<option'.(($params_datetime['time_zone'] == $key) ? ' selected="selected"' : '').' value="'.$key.'">'.$val.'</option>';
					}
		    ...
		    <tr valign="top">
				...
				<td colspan="2">
					...
					for($i=0; $i < 7; $i++){
						$tab_content_3 .= '<option'.(($params_datetime['week_start_day'] == ($i+1)) ? ' selected="selected"' : '').' value="'.($i+1).'">'.get_weekday_local($i+1).'</option>';
					}
					$tab_content_3 .= '</select>
				</td>			
		    </tr>
		    <tr valign="top">
				<td>'._PRICE_FORMAT.': <img class="help" src="images/question_mark.png" title="'._PRICE_FORMAT_ALERT.'" alt="help" /></td>
				<td colspan="2">
					<select name="price_format">
					<option '.(($params_datetime["price_format"] == "european") ? "selected" : "").' value="european">1.234,00 (european)</option>
					<option '.(($params_datetime["price_format"] == "american") ? "selected" : "").' value="american">1,234.00 (american)</option>
					</select>
				</td>
		    </tr>
			<tr><td colspan="3" nowrap height="5px"></td></tr>
		...
		$tab_content_4 = '<form id="frmEmailSettings" action="index.php?admin=settings" method="post">
			...
			<tr valign="top">
				<td width="90px" height="50px">'._MAILER.': <img class="help" src="images/question_mark.png" title="'._ADMIN_MAILER_ALERT.'" alt="help" /></td>
				<td width="140px">
					<input class="form_radio" type="radio" name="mailer" id="mailer_php" '.(($params_email['mailer'] == 'php') ? 'checked="checked"' : '').' value="php" onclick="appHideElement(\'mailer_smtp_row\');appShowElement(\'mailer_php_row\');" /> <label for="mailer_php">PHP Mail Function</label><br />
					<input class="form_radio" type="radio" name="mailer" id="mailer_smtp" '.(($params_email['mailer'] == 'smtp') ? 'checked="checked"' : '').' value="smtp" onclick="appHideElement(\'mailer_php_row\');appShowElement(\'mailer_smtp_row\');" /> <label for="mailer_smtp">SMTP</label><br />
				</td>
				<td style="border-left:1px solid #d1d2d3;padding:2px 18px 2px 18px" rowspan="2">
					...
					<tr valign="top">
						<td width="150px">'._EMAIL_ADDRESS.'<span class="required">*</span>: <img class="help" src="images/question_mark.png" title="'._ADMIN_EMAIL_ALERT.'" alt="help" /></td>
						<td><input type="text" name="admin_email" id="admin_email" size="25" maxlength="70" value="'.$params_email['admin_email'].'" /></td>
					</tr>
					</table>
					
					<table id="mailer_php_row" style="'.(($params_email['mailer'] == 'php') ? '' : 'display:none;').'" border="0" cellspacing="2" cellpadding="2" class="main_text">
					<tr valign="top">
						<td width="150px">'._TYPE.':</td>
						<td>
							<select name="mailer_type">
								<option'.(($params_email['mailer_type'] == 'php_mail_standard') ? ' selected="selected"' : '').' value="php_mail_standard">'._STANDARD.'</option>
								<option'.(($params_email['mailer_type'] == 'php_mail_simple') ? ' selected="selected"' : '').' value="php_mail_simple">'._SIMPLE.'</option>	
							</select>										
						</td>
					</tr>
					<tr><td colspan="2" nowrap height="15px"></td></tr>
					</table>
					<table id="mailer_smtp_row" style="'.(($params_email['mailer'] == 'smtp') ? '' : 'display:none;').'" border="0" cellspacing="2" cellpadding="2" class="main_text">
					<tr valign="top">
						<td width="150px">'._SMTP_AUTH.' <span class="required">*</span>:</td>
						<td>
							<select name="smtp_auth">
								<option'.(($params_email['smtp_auth'] == '0') ? ' selected="selected"' : '').' value="0">'._NO.'</option>
								<option'.(($params_email['smtp_auth'] == '1') ? ' selected="selected"' : '').' value="1">'._YES.'</option>	
							</select>																
						</td>
					</tr>
					<tr valign="top">
						...
						<td>
							<select name="smtp_secure">
								<option'.(($params_email['smtp_secure'] == 'ssl') ? ' selected="selected"' : '').' value="ssl">SSL</option>
								<option'.(($params_email['smtp_secure'] == 'tls') ? ' selected="selected"' : '').' value="tls">TLS</option>
								<option'.(($params_email['smtp_secure'] == 'no') ? ' selected="selected"' : '').' value="no">'._NO.'</option>	
							</select>																
						</td>
					</tr>
					<tr valign="top">
						<td width="150px">'._SMTP_HOST.' <span class="required">*</span>: <img class="help" src="images/question_mark.png" title="Ex.: smtp.gmail.com" alt="help" /></td>
						<td><input type="text" name="smtp_host" id="smtp_host" size="20" maxlength="70" value="'.$params_email['smtp_host'].'" /></td>
					</tr>
					<tr valign="top">
						<td width="150px">'._SMTP_PORT.' <span class="required">*</span>: <img class="help" src="images/question_mark.png" title="Ex.: 465 or 587" alt="help" /></td>
						<td><input type="text" name="smtp_port" id="smtp_port" size="6" maxlength="6" value="'.$params_email['smtp_port'].'" /></td>
					</tr>
					<tr valign="top">
						<td width="150px">'._USERNAME.' <span class="required">*</span>:</td>
						<td><input type="text" name="smtp_username" id="smtp_username" size="20" maxlength="40" value="'.$params_email['smtp_username'].'" autocomplete="off" /></td>
					</tr>
					<tr valign="top">
						<td width="150px">'._PASSWORD.' <span class="required">*</span>:</td>
                        <td><input type="password" name="smtp_password" id="smtp_password" size="20" maxlength="50" value="" '.($objSettings->GetParameter('smtp_password') ? 'placeholder="&bull;&bull;&bull;&bull;&bull;"' : '').' autocomplete="off" /></td>
					</tr>
					</table>
			...
			<tr valign="top">
				...
				<td width="140px">
					<select name="mailer_wysiwyg_type">
						<option'.(($params_email['mailer_wysiwyg_type'] == 'none') ? ' selected="selected"' : '').' value="none">'._NONE.'</option>
						<option'.(($params_email['mailer_wysiwyg_type'] == 'tinymce') ? ' selected="selected"' : '').' value="tinymce">TinyMCE</option>
					</select>
				</td>
			</tr>
			<tr>
		...
		$provider_template_icon = isset($xml->icon) ? 'templates/'.$template.'/'.$xml->icon : 'images/no_image.png';
		$member_template_icon = isset($xml->icon) ? 'templates/'.$template.'/'.$xml->icon : 'images/no_image.png';
		...
		'.draw_token_field(false).'
			...
			<tr valign="top">
				...
				<td width="148px">
					<select name="site_template" onchange="change_icon(this.value,\''.Application::Get('token').'\')">
				...
				<td style="border-left:1px solid #d1d2d3; padding:2px 28px 2px 28px">
					<img id="template_icon" src="'.$template_icon.'" style="border:1px solid #cccccc" alt="template" width="240" height="180px" />				
				</td>
				...
				<tr valign="top">
				<td width="90px">'._PROVIDER_HDR_TEMPLATE.' <span class="required">*</span>:</td>
				<td width="148px">
					<select name="provider_template" onchange="change_icon(this.value,\''.Application::Get('token').'\')">
					<option value="">-- '._SELECT.' --</option>';					
						// prepare templates
						$arr_templates = @read_directory_subfolders('templates/'); 
						foreach($arr_templates as $key){
							$tab_content_5 .= '<option'.(($provider_template == $key) ? ' selected="selected"' : '').' value="'.$key.'">'.ucfirst($key).'</option>';
						}
						$tab_content_5 .= '
					</select>
				</td>
				<td style="border-left:1px solid #d1d2d3; padding:2px 28px 2px 28px">
					<img id="template_icon" src="'.$provider_template_icon.'" style="border:1px solid #cccccc" alt="template" width="240" height="180px" />				
				</td>
				<td style="min-width:400px">
					<img class="loading_img" src="images/ajax_loading.gif" alt="loading..." />
					<b>'._NAME.'</b>: <span id="template_name">'.$template_name.'</span><br />
			...
			<tr valign="top">
				<td width="90px">'._MEMBER_HDR_TEMPLATE.' <span class="required">*</span>:</td>
				<td width="148px">
					<select name="member_template" onchange="change_icon(this.value,\''.Application::Get('token').'\')">
					<option value="">-- '._SELECT.' --</option>';					
						// prepare templates
						$arr_templates = @read_directory_subfolders('templates/'); 
						foreach($arr_templates as $key){
							$tab_content_5 .= '<option'.(($member_template == $key) ? ' selected="selected"' : '').' value="'.$key.'">'.ucfirst($key).'</option>';
						}
						$tab_content_5 .= '
					</select>
				</td>
				<td style="border-left:1px solid #d1d2d3; padding:2px 28px 2px 28px">
					<img id="template_icon" src="'.$member_template_icon.'" style="border:1px solid #cccccc" alt="template" width="240" height="180px" />				
				</td>
				<td style="min-width:400px">
					<img class="loading_img" src="images/ajax_loading.gif" alt="loading..." />
					<b>'._NAME.'</b>: <span id="template_name">'.$template_name.'</span><br />
		...
		$tab_content_6 = '<form id="frmEmailSettings" action="index.php?admin=settings" method="post">
			'.draw_hidden_field('submition_type', 'social_networks', false).'
			'.draw_hidden_field('tabid', $tabid, false).'
			'.draw_token_field(false).'		

			<table width="100%" border="0" cellspacing="5" cellpadding="5" class="main_text">
			<tr valign="top">
                <td width="110px" colspan="2">'._SOCIAL_NETWORKS.': </td>
                <td style="border-left:1px solid #d1d2d3;padding:2px 18px 2px 18px">
                    <table border="0" cellspacing="2" cellpadding="2" class="main_text">';
    
                    foreach($social_networks as $key => $val){
                        $tab_content_6 .= '<tr valign="middle">
                            <td width="90px">'.$val['name'].'</td>
                            <td>
                                '.(($val['url'] != '') ? '<a href="'.$val['url'].'" target="_new">' : '').'
                                <img class="social_icon" src="images/social_networks/'.$key.'.png" alt="Dribbble" />
                                '.(($val['url'] != '') ? '</a>' : '').'
                                &nbsp;<input type="text" name="link_'.$key.'" id="link_'.$key.'" style="width:370px" maxlength="150" value="'.decode_text($params_social['link_'.$key]).'" placeholder="http://" />
                            </td>
                        </tr>';
                    }
                    
                    $tab_content_6 .= '<tr><td colspan="3" nowrap height="10px"></tr>
                    <tr>
		...
		$tab_content_7 = '<table width="99%" border="0" cellspacing="5" cellpadding="5" class="main_text"><tr valign="top"><td>';
		$tab_content_7 .= '<ul '.((Application::Get('lang_dir') == 'rtl') ? 'style="list-style:none"' : '').'>
				<li><b>PHP Version:</b> <i>'.$phpversion.'</i></li>
				<li><b>'.((DB_TYPE == 'PDO') ? 'PDO MySQL' : DB_TYPE).' Version:</b> <i>'.(isset($mysql_version['ve']) ? $mysql_version['ve'] : _UNKNOWN).'</i></li>
...
&lt;/Files&gt;
	...
	$tab1=$tabs->AddTab(_GENERAL, $tab_content_1);
	...
	$tab6=$tabs->AddTab(_SOCIAL_NETWORKS, $tab_content_6);
