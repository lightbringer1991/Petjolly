...
if($objLogin->IsLoggedInAsAdmin()){
	...
	$provider_template  = isset($_POST['provider_template']) ? prepare_input($_POST['provider_template']) : '';
	$member_template  = isset($_POST['member_template']) ? prepare_input($_POST['member_template']) : '';
	...
	if($submition_type == 'general'){
		...
		if(!empty($provider_template)){
			if($objSettings->SetProviderTemplate($provider_template) == true){
				delete_cache();
				$msg = draw_success_message(_SETTINGS_SAVED, false);	
			}else $msg = draw_important_message($objSettings->error, false);									
		}else $msg = draw_important_message(_TEMPLATE_IS_EMPTY, false);
		if(!empty($member_template)){
			if($objSettings->SetMemberTemplate($member_template) == true){
				delete_cache();
				$msg = draw_success_message(_SETTINGS_SAVED, false);	
			}else $msg = draw_important_message($objSettings->error, false);									
		}else $msg = draw_important_message(_TEMPLATE_IS_EMPTY, false);
	...
	$provider_template = $objSettings->GetProviderTemplate();	
	$member_template = $objSettings->GetMemberTemplate();	
